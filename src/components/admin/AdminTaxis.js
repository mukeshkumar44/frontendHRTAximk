  ...
  useEffect(() => { 
    ...
    const fetchTaxis = async () => { 
      try { 
        ...
        // Ensure we're setting an array, even if response.data is undefined
        setTaxis(Array.isArray(response?.data?.data) ? response.data.data : []); 
      } catch (err) { 
        console.error('Error fetching taxis:', err);
        setError('Failed to load taxis. Please try again later.');
        setTaxis([]); // Ensure taxis is always an array
      } finally { 
        setLoading(false); 
      } 
    }; 
